execute as @e[tag=1] at @e run clear @a porkchop{display:{Name:'{"text":"Domuz Eti","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:porkchop 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:porkchop 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"x32","bold":true,"color":"gold"},{"text":" Domuz Eti başarıyla satıldı!","color":"yellow"}]
