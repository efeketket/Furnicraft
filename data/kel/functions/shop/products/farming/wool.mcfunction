execute as @e[tag=1] at @e run clear @a white_wool{display:{Name:'{"text":"Yün","bold":true,"italic":false}'}} 16
...
execute as @s[scores={selling=1}] run clear @s minecraft:white_wool 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"x16","bold":true,"color":"gold"},{"text":" Yün başarıyla satıldı!","color":"yellow"}]
