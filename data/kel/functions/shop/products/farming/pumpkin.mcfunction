execute as @e[tag=1] at @e run clear @a pumpkin{display:{Name:'{"text":"Balkabağı","bold":true,"italic":false}'}} 32
...
execute as @s[scores={selling=1}] run clear @s minecraft:pumpkin 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"x32","bold":true,"color":"gold"},{"text":" Balkabağı başarıyla satıldı!","color":"yellow"}]
