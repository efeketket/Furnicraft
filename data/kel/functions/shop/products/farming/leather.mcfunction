execute as @e[tag=1] at @e run clear @a leather{display:{Name:'{"text":"Deri","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:leather 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:leather 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"x32","bold":true,"color":"gold"},{"text":" Deri başarıyla satıldı!","color":"yellow"}]
