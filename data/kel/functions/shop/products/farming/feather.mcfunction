execute as @e[tag=1] at @e run clear @a feather{display:{Name:'{"text":"Tüy","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:feather 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:feather 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"x32","bold":true,"color":"gold"},{"text":" Tüy başarıyla satıldı!","color":"yellow"}]
