execute as @e[tag=1] at @e run clear @a stone{display:{Name:'{"text":"Taş","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:stone 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:stone 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"64x","bold":true,"color":"gold"},{"text":" Taş başarıyla satıldı!","color":"yellow"}]
