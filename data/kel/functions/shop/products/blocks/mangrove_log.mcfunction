execute as @e[tag=1] at @e run clear @a mangrove_log{display:{Name:'{"text":"Mangrov Kütüğü","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:mangrove_log 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:mangrove_log 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Mangrov Kütüğü başarıyla satıldı!","color":"yellow"}]
