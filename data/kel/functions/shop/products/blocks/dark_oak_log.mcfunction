execute as @e[tag=1] at @e run clear @a dark_oak_log{display:{Name:'{"text":"Koyu Meşe Kütüğü","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:dark_oak_log 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:dark_oak_log 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"32x","bold":true,"color":"gold"},{"text":" Koyu Meşe Kütüğü başarıyla satıldı!","color":"yellow"}]
