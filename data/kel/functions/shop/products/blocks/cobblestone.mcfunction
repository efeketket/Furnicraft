execute as @e[tag=1] at @e run clear @a cobblestone{display:{Name:'{"text":"Kırık Taş","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:cobblestone 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cobblestone 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"64x","bold":true,"color":"gold"},{"text":" Kırık Taş başarıyla satıldı!","color":"yellow"}]
