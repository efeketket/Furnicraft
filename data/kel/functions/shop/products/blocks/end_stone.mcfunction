execute as @e[tag=1] at @e run clear @a end_stone{display:{Name:'{"text":"End Taşı","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:end_stone 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:end_stone 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"32x","bold":true,"color":"gold"},{"text":" End Taşı başarıyla satıldı!","color":"yellow"}]
