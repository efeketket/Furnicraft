execute as @e[tag=1] at @e run clear @a quartz_block{display:{Name:'{"text":"Kuvars Bloğu","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:quartz_block 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:quartz_block 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Kuvars Bloğu başarıyla satıldı!","color":"yellow"}]
