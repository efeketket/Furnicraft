execute as @e[tag=1] at @e run clear @a sandstone{display:{Name:'{"text":"Kum Taşı","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:sandstone 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:sandstone 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"64x","bold":true,"color":"gold"},{"text":" Kum Taşı başarıyla satıldı!","color":"yellow"}]
