execute as @e[tag=1] at @e run clear @a glowstone{display:{Name:'{"text":"Işık Taşı","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:glowstone 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:glowstone 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Işık Taşı başarıyla satıldı!","color":"yellow"}]
