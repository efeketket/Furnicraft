execute as @e[tag=1] at @e run clear @a cobbled_deepslate{display:{Name:'{"text":"Kırık Derin Kayrak Taşı","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:cobbled_deepslate 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cobbled_deepslate 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"64x","bold":true,"color":"gold"},{"text":" Kırık Kayrak Derin Taşı başarıyla satıldı!","color":"yellow"}]
