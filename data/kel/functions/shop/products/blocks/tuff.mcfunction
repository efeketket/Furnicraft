execute as @e[tag=1] at @e run clear @a tuff{display:{Name:'{"text":"Tuff","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:tuff 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:tuff 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"64x","bold":true,"color":"gold"},{"text":" Tuff başarıyla satıldı!","color":"yellow"}]
