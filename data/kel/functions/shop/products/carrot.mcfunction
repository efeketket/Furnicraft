execute as @e[tag=1] at @e run clear @a carrot{display:{Name:'{"text":"Havuç","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:carrot 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:carrot 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"x64","bold":true,"color":"gold"},{"text":" Havuç başarıyla satıldı!","color":"yellow"}]
