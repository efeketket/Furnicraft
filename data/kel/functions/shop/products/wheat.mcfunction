execute as @e[tag=1] at @e run clear @a wheat{display:{Name:'{"text":"Buğday","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:wheat 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:wheat 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"x64","bold":true,"color":"gold"},{"text":" Buğday başarıyla satıldı!","color":"yellow"}]
