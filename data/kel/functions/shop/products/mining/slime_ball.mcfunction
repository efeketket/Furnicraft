execute as @e[tag=1] at @e run clear @a slime_ball{display:{Name:'{"text":"Balçık Topu","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:slime_ball 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:slime_ball 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Balçık Topu başarıyla satıldı!","color":"yellow"}]
