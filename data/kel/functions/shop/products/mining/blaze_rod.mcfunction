execute as @e[tag=1] at @e run clear @a blaze_rod{display:{Name:'{"text":"Alaz Çubuğu","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:blaze_rod 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:blaze_rod 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Alaz Çubuğu başarıyla satıldı!","color":"yellow"}]
