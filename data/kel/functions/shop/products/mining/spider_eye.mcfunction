execute as @e[tag=1] at @e run clear @a spider_eye{display:{Name:'{"text":"Örümcek Gözü","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:spider_eye 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:spider_eye 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Örümcek Gözü başarıyla satıldı!","color":"yellow"}]
