execute as @e[tag=1] at @e run clear @a amethyst_shard{display:{Name:'{"text":"Ametist Parçası","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:amethyst_shard 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:amethyst_shard 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Ametist Parçası başarıyla satıldı!","color":"yellow"}]
