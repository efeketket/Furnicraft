execute as @e[tag=1] at @e run clear @a glass_bottle{display:{Name:'{"text":"Boş Şişe","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:glass_bottle 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:glass_bottle 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Boş Şişe başarıyla satıldı!","color":"yellow"}]
