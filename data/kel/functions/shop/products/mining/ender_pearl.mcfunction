execute as @e[tag=1] at @e run clear @a ender_pearl{display:{Name:'{"text":"Ender İncisi","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:ender_pearl 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:ender_pearl 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Ender İncisi başarıyla satıldı!","color":"yellow"}]
