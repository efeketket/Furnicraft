execute as @e[tag=1] at @e run clear @a emerald{display:{Name:'{"text":"Zümrüt","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:emerald 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:emerald 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Zümrüt başarıyla satıldı!","color":"yellow"}]
