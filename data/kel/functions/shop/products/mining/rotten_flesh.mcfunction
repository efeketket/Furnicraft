execute as @e[tag=1] at @e run clear @a rotten_flesh{display:{Name:'{"text":"Çürük Et","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:rotten_flesh 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:rotten_flesh 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"64x","bold":true,"color":"gold"},{"text":" Çürük Et başarıyla satıldı!","color":"yellow"}]
