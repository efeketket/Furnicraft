execute as @e[tag=1] at @e run clear @a copper_ingot{display:{Name:'{"text":"Bakır","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:copper_ingot 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:copper_ingot 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Bakır başarıyla satıldı!","color":"yellow"}]
