execute as @e[tag=1] at @e run clear @a coal{display:{Name:'{"text":"Kömür","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:coal 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:coal 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"32x","bold":true,"color":"gold"},{"text":" Kömür başarıyla satıldı!","color":"yellow"}]
