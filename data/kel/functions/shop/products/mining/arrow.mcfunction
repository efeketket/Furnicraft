execute as @e[tag=1] at @e run clear @a arrow{display:{Name:'{"text":"Ok","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:arrow 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:arrow 64
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"64x","bold":true,"color":"gold"},{"text":" Ok başarıyla satıldı!","color":"yellow"}]
