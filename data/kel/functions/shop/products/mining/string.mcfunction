execute as @e[tag=1] at @e run clear @a string{display:{Name:'{"text":"İp","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:string 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:string 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"32x","bold":true,"color":"gold"},{"text":" İp başarıyla satıldı!","color":"yellow"}]
