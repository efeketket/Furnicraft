execute as @e[tag=1] at @e run clear @a nether_wart{display:{Name:'{"text":"Nether Yumrusu","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:nether_wart 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:nether_wart 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Nether Yumrusu başarıyla satıldı!","color":"yellow"}]
