execute as @e[tag=1] at @e run clear @a rabbit{display:{Name:'{"text":"Tavşan Eti","bold":true,"italic":false}'}} 8
execute as @s store result score @s item_count run clear @s minecraft:rabbit 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:rabbit 8
execute as @s[scores={selling=1}] run tellraw @p ["",{"text":"x8","bold":true,"color":"gold"},{"text":" Tavşan Eti başarıyla satıldı!","color":"yellow"}]
