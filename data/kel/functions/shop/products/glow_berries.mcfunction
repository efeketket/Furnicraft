execute as @e[tag=1] at @e run clear @a glow_berries{display:{Name:'{"text":"Parıldayan Meyve","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:glow_berries 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:glow_berries 32
execute as @s[scores={selling=1}] run tellraw @p ["",{"text":"x32","bold":true,"color":"gold"},{"text":" Parıldayan Meyve başarıyla satıldı!","color":"yellow"}]
