execute as @e[tag=1] at @e run clear @a chorus_fruit{display:{Name:'{"text":"Nakarat Meyvesi","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:chorus_fruit 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:chorus_fruit 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"32x","bold":true,"color":"gold"},{"text":" Nakarat Meyvesi başarıyla satıldı!","color":"yellow"}]
