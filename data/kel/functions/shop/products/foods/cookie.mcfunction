execute as @e[tag=1] at @e run clear @a cookie{display:{Name:'{"text":"Kurabiye","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:cookie 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cookie 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Kurabiye başarıyla satıldı!","color":"yellow"}]
