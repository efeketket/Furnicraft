execute as @e[tag=1] at @e run clear @a golden_apple{display:{Name:'{"text":"Altın Elma","bold":true,"italic":false}'}} 2
execute as @s store result score @s item_count run clear @s minecraft:golden_apple 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:golden_apple 2
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"2x","bold":true,"color":"gold"},{"text":" Altın Elma başarıyla satıldı!","color":"yellow"}]
