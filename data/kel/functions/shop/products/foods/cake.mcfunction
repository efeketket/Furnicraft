execute as @e[tag=1] at @e run clear @a cake{display:{Name:'{"text":"Pasta","bold":true,"italic":false}'}} 1
execute as @s store result score @s item_count run clear @s minecraft:cake 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cake 1
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"1x","bold":true,"color":"gold"},{"text":" Pasta başarıyla satıldı!","color":"yellow"}]
