execute as @e[tag=1] at @e run clear @a dried_kelp{display:{Name:'{"text":"Kurutulmuş Yosun","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:dried_kelp 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:dried_kelp 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"32x","bold":true,"color":"gold"},{"text":" Kurutulmuş Yosun başarıyla satıldı!","color":"yellow"}]
