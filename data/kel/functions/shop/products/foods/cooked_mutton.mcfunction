execute as @e[tag=1] at @e run clear @a cooked_mutton{display:{Name:'{"text":"Pişmiş Koyun Eti","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:cooked_mutton 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cooked_mutton 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Pişmiş Koyun Eti başarıyla satıldı!","color":"yellow"}]
