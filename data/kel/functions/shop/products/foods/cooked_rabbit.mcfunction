execute as @e[tag=1] at @e run clear @a cooked_rabbit{display:{Name:'{"text":"Pişmiş Tavşan Eti","bold":true,"italic":false}'}} 4
execute as @s store result score @s item_count run clear @s minecraft:cooked_rabbit 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cooked_rabbit 4
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"4x","bold":true,"color":"gold"},{"text":" Pişmiş Tavşan Eti başarıyla satıldı!","color":"yellow"}]
