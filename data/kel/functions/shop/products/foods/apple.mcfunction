execute as @e[tag=1] at @e run clear @a apple{display:{Name:'{"text":"Elma","bold":true,"italic":false}'}} 16
execute as @s store result score @s item_count run clear @s minecraft:apple 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:apple 16
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"16x","bold":true,"color":"gold"},{"text":" Elma başarıyla satıldı!","color":"yellow"}]
