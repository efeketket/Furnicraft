execute as @e[tag=1] at @e run clear @a cooked_salmon{display:{Name:'{"text":"Pişmiş Somon Balığı","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:cooked_salmon 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cooked_salmon 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"32x","bold":true,"color":"gold"},{"text":" Pişmiş Somon Balığı başarıyla satıldı!","color":"yellow"}]
