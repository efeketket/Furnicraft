execute as @e[tag=1] at @e run clear @a pumpkin_pie{display:{Name:'{"text":"Balkabağı Pastası","bold":true,"italic":false}'}} 8
execute as @s store result score @s item_count run clear @s minecraft:pumpkin_pie 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:pumpkin_pie 8
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"8x","bold":true,"color":"gold"},{"text":" Balkabağı Pastası başarıyla satıldı!","color":"yellow"}]
