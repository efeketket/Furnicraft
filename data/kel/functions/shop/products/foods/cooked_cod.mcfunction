execute as @e[tag=1] at @e run clear @a cooked_cod{display:{Name:'{"text":"Pişmiş Morina Balığı","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:cooked_cod 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cooked_cod 32
execute as @s[scores={selling=1}] run tellraw @s ["",{"text":"32x","bold":true,"color":"gold"},{"text":" Pişmiş Morina Balığı başarıyla satıldı!","color":"yellow"}]
