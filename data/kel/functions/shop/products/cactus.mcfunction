execute as @e[tag=1] at @e run clear @a cactus{display:{Name:'{"text":"Kaktüs","bold":true,"italic":false}'}} 64
execute as @s store result score @s item_count run clear @s minecraft:cactus 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:cactus 64
execute as @s[scores={selling=1}] run tellraw @p ["",{"text":"x32","bold":true,"color":"gold"},{"text":" Cactus aşarıyla satıldı!","color":"yellow"}]
