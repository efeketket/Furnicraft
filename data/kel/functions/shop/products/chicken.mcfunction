execute as @e[tag=1] at @e run clear @a chicken{display:{Name:'{"text":"Tavuk Eti","bold":true,"italic":false}'}} 32
execute as @s store result score @s item_count run clear @s minecraft:chicken 0
...
execute as @s[scores={selling=1}] run clear @s minecraft:chicken 32
execute as @s[scores={selling=1}] run tellraw @p ["",{"text":"x32","bold":true,"color":"gold"},{"text":" Tavuk başarıyla satıldı!","color":"yellow"}]
